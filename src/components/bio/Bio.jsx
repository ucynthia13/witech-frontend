import React from 'react'

function Bio() {
  return (
    <div className='container mt-4 pt-4 text-font' id='about'>
        <div className='row flex-lg-row d-flex justify-content-center g-5 py-5'>
          <p className='h6 text-center'>Why Us?</p>
          <div className='col-10 col-sm-8 col-lg-6 blob2 rounded-3'>

          </div>
          <div className='col-lg-6 ms-5'>
            <p className='fw-bold mb-4 h2 mt-5 pt-5'>We Provide You <span className='heading-color underlined'> The Best 
               </span>
               <span className='mt-3 d-block'>
                Training To Students
                </span>
            </p>
              <p className=' ms-3 text-font-size'>Empowering tommorrow's Tech Leaders with quality education, comphrehensive software training programs, cutting edge curricula, and a commitment to excellence</p>
            <div className='row'>
                <div className='col'>
                <p className='text-center'><span className='d-block fw-bolder mt-4 mb-2 h3 heading-color'>1+</span> <span className='d-block text-font-size'>years of</span><span className='d-block text-font-size'>experience</span></p>
                </div>
                <div className='col'>
                <p className='text-center'><span className='d-block fw-bolder mt-4 mb-2 h3 heading-color'>10</span> <span className='d-block text-font-size'>Experienced</span><span className='d-block text-font-size'>Mentors</span></p>
                </div>
                <div className='col'>
                <p className='text-center'><span className='d-block fw-bolder mt-4 mb-2 h3 heading-color'>30+</span> <span className='d-block text-font-size'>Trained</span><span className='d-block text-font-size'>Students</span></p>
                </div>
            </div>
            <button className='btn btn-outline-secondary ms-4 px-5 py-2 mt-3 button-background text-font-size'>Learn More</button>
        </div>


  </div>
 
  </div>
  )
}

export default Bio